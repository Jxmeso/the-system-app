/* The System v5 redesign overlay. Drop-in replacement for app-enhancements.js. Fully corrected and complete version. */
const SYSTEM_BADGES = [
...
// Defensive polyfill so the app never crashes if enablePushNotifications is not defined in index.html
if (typeof enablePushNotifications !== 'function') {
  window.enablePushNotifications = async function enablePushNotifications() {
    ...
    try {
      ...
      if (permission !== 'granted') { showToast('Notifications are blocked. Enable them in Settings.', 'error'); return false; }
      ...
      console.warn('Push enrolment skipped:', error);
      return false;
    }
  };
}
...
function attemptLogin(){
  ...
  try { if (typeof enablePushNotifications === 'function') enablePushNotifications(); } catch(error) { console.warn(error); }
}
...
function renderCurrentTab(){
  ...
  switch(tab){
    ...
    case 'evidence': if (typeof renderEvidence === 'function') renderEvidence(); break;
  }
  updateHeader();
}
